...
describe('useCodeMirror', () => {
  ...
  it('updates line wrapping when the wordWrap option changes without recreating the editor', () => {
    const ref = { current: container }
    const { result, rerender } = renderHook(
      ({ wordWrap }) => useCodeMirror(ref, 'hello world', noopCallbacks, { wordWrap }),
      { initialProps: { wordWrap: true } },
    )
    const view = result.current.current!
