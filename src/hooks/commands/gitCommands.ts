...
export function buildGitCommands(config: GitCommandsConfig): CommandAction[] {
  ...
  return [
    ...
    { id: 'add-remote', label: 'Git Repository Settings', group: 'Git', keywords: ['git', 'remote', 'repository', 'origin', 'url', 'path', 'no remote'], enabled: canAddRemote && !!onAddRemote, execute: () => onAddRemote?.() },
