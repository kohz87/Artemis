...
beforeEach(() => {
  ...
  vi.useFakeTimers()
  vi.stubGlobal('fetch', vi.fn().mockRejectedValue(new Error('not configured')))
})
...
describe('useAiActivity', () => {
  ...
  it('keeps using the resolved websocket URL after reconnects', async () => {
    vi.mocked(fetch).mockResolvedValue({
      ok: true,
      json: async () => ({ uiWsUrl: 'ws://localhost:8123' }),
    } as Response)
    ...
    const resolvedWs = wsInstances[wsInstances.length - 1]
    expect(resolvedWs?.url).toBe('ws://localhost:8123')
    ...
    expect(wsInstances[wsInstances.length - 1]?.url).toBe('ws://localhost:8123')
  })
})
