...
describe('useSettings', () => {
  ...
  it('clears stored UI language preferences on load', async () => {
    mockSettingsStore = {
      ...savedSettings,
      ui_language: 'zh-CN',
    }
  ...
  it('preserves word wrap and note width override preferences', async () => {
    mockSettingsStore = {
      ...savedSettings,
      word_wrap_enabled: false,
      note_width_overrides: { 'Notes/Long.md': 'wide' },
    }

    const settings = await renderLoadedSettings()

    expect(settings.word_wrap_enabled).toBe(false)
    expect(settings.note_width_overrides).toEqual({ 'Notes/Long.md': 'wide' })
  })
