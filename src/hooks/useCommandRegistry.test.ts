...
function expectFolderCommandStates(overrides: Record<string, unknown>, expected: {
  ...
  expect(findCommand(result.current, 'reveal-selected-folder')).toBeUndefined()
...
describe('useCommandRegistry', () => {
  ...
  it('exposes active file actions when a note is selected', () => {
    ...
    expect(findCommand(result.current, 'reveal-active-file')).toBeUndefined()
  ...
  it('omits the OS external-open file command', () => {
    ...
    expect(findCommand(result.current, 'open-active-file-external')).toBeUndefined()
  })
  ...
  it('enables folder commands when a folder is selected', () => {
    ...
    }, { copy: true, delete: true, rename: true })
  })
  ...
  it('disables folder commands outside folder selection', () => {
    expectFolderCommandStates({
      ...
      onDeleteFolder: vi.fn(),
      onCopySelectedFolderPath: vi.fn(),
    }, { copy: false, delete: false, rename: false })
  })

  it('keeps root folder copy command enabled without destructive actions', () => {
    expectFolderCommandStates({
      selection: { kind: 'folder', path: '', rootPath: '/Users/luca/Laputa' },
      onRenameFolder: vi.fn(),
      onDeleteFolder: vi.fn(),
      onCopySelectedFolderPath: vi.fn(),
    }, { copy: true, delete: false, rename: false })
  })
  ...
  it('omits the removed Contribute command', () => {
    const config = makeConfig()
    const { result } = renderHook(() => useCommandRegistry(config))
