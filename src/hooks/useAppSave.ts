...
function useRenameHandlers({
  ...
  const handleFilenameRename = useCallback(async (path: string, newFilenameStem: string, options?: FilenameRenameOptions) => {
    ...
    const rename = options?.syncOnly
      ? handleRenameFilename(currentPath, newFilenameStem, resolvedPath, replaceRenamedEntry)
      : handleRenameNote(currentPath, filenameStemToTitle(newFilenameStem), resolvedPath, replaceRenamedEntry)
    await rename.then(loadModifiedFiles)
  }, [resolveCurrentPath, savePendingForPath, cancelPendingUntitledRename, handleRenameFilename, handleRenameNote, resolvedPath, replaceRenamedEntry, loadModifiedFiles])
