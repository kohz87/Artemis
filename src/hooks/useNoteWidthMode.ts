...
interface PersistWidthRequest {
  ...
  persistSettingsWidthOverride: (path: VaultPath, mode: NoteWidthMode) => Promise<void>
  setToastMessage: (message: ToastMessage) => void
}
...
async function persistOrRememberNoteWidth({
  ...
  if (!canPersistNoteWidthMode(persistedContent)) {
    try {
      ...
      await persistSettingsWidthOverride(path, mode)
      trackEvent('note_width_mode_changed', { mode, scope: 'settings' })
    } catch (err) {
...
export function useNoteWidthMode({
  ...
  const [transientNoteWidths, setTransientNoteWidths] = useState<Record<VaultPath, NoteWidthMode>>(() => settings.note_width_overrides ?? {})

  useEffect(() => {
    const nextWidths = settings.note_width_overrides ?? {}
    let cancelled = false

    queueMicrotask(() => {
      if (!cancelled) setTransientNoteWidths(nextWidths)
    })

    return () => {
      cancelled = true
    }
  }, [settings.note_width_overrides])
  ...
  const rememberTransientWidth = useCallback((path: VaultPath, mode: NoteWidthMode) => {
    setTransientNoteWidths((previous) => ({ ...previous, [path]: mode }))
  }, [])

  const persistSettingsWidthOverride = useCallback(async (path: VaultPath, mode: NoteWidthMode) => {
    const nextOverrides = { ...(settings.note_width_overrides ?? {}), [path]: mode }
    await saveSettings({ ...settings, note_width_overrides: nextOverrides })
  ...
  const setNoteWidth = useCallback((mode: NoteWidthMode) => persistOrRememberNoteWidth({
    ...
    persistSettingsWidthOverride,
    setToastMessage,
  }), [activeTab, persistSettingsWidthOverride, rememberTransientWidth, setToastMessage, updateFrontmatter])
