...
const PULL_PUSH_RECOVERY_ATTEMPTS = 2
...
function handlePushResult(options: {
  ...
  if (pushResult.status === 'rejected') {
    ...
    void callbacksRef.current.onToast(formatPushRejectedAfterPull(pushResult))
    return
  }
  setSyncStatus('error')
...
function formatPushRejectedAfterPull(pushResult: GitPushResult): string {
  const message = pushResult.message.replace(/^push rejected:?\s*/i, '').trim()
  if (!message) return 'Push still rejected after pull'
  return `Push still rejected after pull: ${message}`
}
