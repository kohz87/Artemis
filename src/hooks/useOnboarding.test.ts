...
describe('useOnboarding', () => {
  ...
  it('opens an existing folder and transitions to ready', async () => {
    ...
    expect(registerVault).toHaveBeenCalledWith('/selected/folder', 'folder', { verifyAvailability: false })
    expect(localStorage.getItem(APP_STORAGE_KEYS.welcomeDismissed)).toBe('1')
  })
  ...
  it('shows the native picker unavailable message instead of crashing the welcome flow', async () => {
    mockCommands()
    ...
    expect(result.current.error).toBe(NATIVE_FOLDER_PICKER_UNAVAILABLE_MESSAGE)
    expect(result.current.state.status).toBe('welcome')
  })
