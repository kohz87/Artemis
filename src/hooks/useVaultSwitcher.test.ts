...
describe('useVaultSwitcher', () => {
  ...
  it('persists the default vault as the active selection on startup', async () => {
    const { result } = renderHook(() => useVaultSwitcher({ onSwitch, onToast }))
    ...
    expect(result.current.selectedVaultPath).toBe(expectedDefaultVaultPath)
    await waitFor(() => {
      expect(mockVaultListStore.active_vault).toBe(expectedDefaultVaultPath)
    })
  })
  ...
  it('shows a clear toast when folder picking is blocked', async () => {
    const { pickFolder } = await import('../utils/vault-dialog')
    ...
    expect(onToast).toHaveBeenCalledWith(NATIVE_FOLDER_PICKER_UNAVAILABLE_MESSAGE)
  })
  ...
  describe('default vault path', () => {
    ...
    it('treats a remembered default vault as the active startup selection when it is the only vault', async () => {
      mockVaultListStore = {
        vaults: [],
        active_vault: expectedDefaultVaultPath,
        hidden_defaults: [],
