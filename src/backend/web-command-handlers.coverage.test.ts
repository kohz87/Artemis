...
describe('mockHandlers coverage', () => {
  ...
  it('does not invent modified-file listings or commits in the browser fallback', async () => {
    const { mockHandlers } = await loadHandlers()
    ...
    expect(() => mockHandlers.git_commit({ message: 'Save everything' })).toThrow('Git commit is unavailable without the vault API')
    expect(mockHandlers.get_modified_files()).toEqual([])
  })

  it('searches mock content and leaves pulse empty without the vault API', async () => {
    const { mockHandlers } = await loadHandlers()
