...
async function loadHandlers() {
  vi.resetModules()
  return import('./web-command-handlers')
}
...
describe('mockHandlers additional coverage', () => {
  ...
  it('returns entry fallbacks, empty fallback history, empty fallback diffs, and empty search results for empty queries', async () => {
    ...
    expect(mockHandlers.get_file_history({ path: '/vault/notes/strategy.md' })).toEqual([])

    expect(mockHandlers.get_file_diff({ path: '/vault/old-draft.md' })).toBe('')
  ...
  it('surfaces the simple command handlers for git, conflicts, trash, and telemetry', async () => {
    ...
    expect(mockHandlers.git_pull()).toEqual({
      status: 'error',
      message: 'Git pull is unavailable without the vault API',
      updatedFiles: [],
      conflictFiles: [],
    })
    expect(mockHandlers.git_push()).toEqual({
      status: 'error',
      message: 'Git push is unavailable without the vault API',
    })
