...
describe('WikilinkChatInput', () => {
  ...
  it('lets committed composed characters reach the native input pipeline once', () => {
    ...
    const cjkText = `${portugueseText}你`
    const cjkKey = createEvent.keyDown(editor, { key: '你' })
    fireEvent(editor, cjkKey)
    expect(cjkKey.defaultPrevented).toBe(false)

    editor.textContent = cjkText
    setSelection(editor, cjkText.length)
    fireEvent.input(editor)
    expect(onDraftChange).toHaveBeenLastCalledWith(cjkText)
  })
  ...
  it('treats file://localhost Windows uri drops as local paths', () => {
    expect(extractDroppedPathText(
      createFileLikeDataTransfer({
        uriList: 'file://localhost/C:/Users/test/My%20Folder',
      }) as DataTransfer,
    )).toBe('"C:/Users/test/My Folder"')
  })
