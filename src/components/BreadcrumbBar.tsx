...
import type { FilenameRenameOptions, NoteWidthMode, VaultEntry } from '../types'
  ...
  return (
    <IconActionButton
      copy={{
        ...
        shortcut: formatShortcutDisplay({ display: 'Ctrl+Backspace / Ctrl+Delete' }),
      }}
      onClick={onDelete}
...
function FilePathActions({
  ...
  locale = 'en',
  onCopyFilePath,
}: Pick<BreadcrumbBarProps, 'entry' | 'locale' | 'onCopyFilePath'>) {
  return (
    <>
      {onCopyFilePath && (
        <IconActionButton
  ...
  return (
    <IconActionButton
      copy={{
        ...
        shortcut: getAppCommandShortcutDisplay(APP_COMMAND_IDS.viewToggleProperties),
      }}
  ...
  return (
    <ActionTooltip copy={{ label: translate(locale, 'editor.filename.renameToTitle') }} side="bottom">
      <Button
        ...
        onClick={() => onRenameFilename(entryPath, syncStem, { syncOnly: true })}
...
function BreadcrumbActions({
  ...
  return (
    ...
    >
      ...
      <OverflowToolbarAction>
        <FilePathActions entry={entry} locale={locale} onCopyFilePath={onCopyFilePath} />
      </OverflowToolbarAction>
      <OverflowToolbarAction>
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="min-w-44">
        {runDiffAction && (
          <DropdownMenuItem onSelect={runDiffAction}>
            <GitBranch size={16} />
            {diffLabel}
          </DropdownMenuItem>
        )}
        {onToggleNoteWidth && (
          <DropdownMenuItem onSelect={onToggleNoteWidth}>
        ...
        {runCopyPathAction && (
          <DropdownMenuItem onSelect={runCopyPathAction}>
            <ClipboardText size={16} />
            {translate(locale, 'editor.toolbar.copyFilePath')}
          </DropdownMenuItem>
        )}
        {runArchiveAction && (
          <DropdownMenuItem onSelect={runArchiveAction}>
...
function BreadcrumbTitle({
  ...
  return (
    <div className="breadcrumb-bar__title-content flex items-center gap-1.5 min-w-0 text-sm text-muted-foreground">
      ...
      <span className="shrink-0 text-border">&rsaquo;</span>
