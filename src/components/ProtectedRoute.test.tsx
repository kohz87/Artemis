...
import { ARTEMIS_AUTH_SESSION_KEY, type AuthSession } from '../hooks/useAuth'
...
describe('ProtectedRoute', () => {
  ...
  it('shows an error after a wrong password and allows retry', () => {
    vi.stubEnv('ARTEMIS_PASSWORD', 'swordfish')
  ...
  it('keeps app access across browser restarts and supports logout', () => {
    vi.stubEnv('ARTEMIS_PASSWORD', 'swordfish')
  ...
  it('allows users to manually clear the persistent session', () => {
    vi.stubEnv('ARTEMIS_PASSWORD', 'swordfish')
    localStorage.setItem(ARTEMIS_AUTH_SESSION_KEY, JSON.stringify({
      authenticated: true,
      session_created_at: now.toISOString(),
