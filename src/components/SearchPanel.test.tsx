...
describe('SearchPanel', () => {
  ...
  it('navigates results with arrow keys', async () => {
    callWebBackendFn.mockResolvedValue({
      results: [
        ...
        { title: 'Result Three', path: '/vault/other/third.md', snippet: 'Third result', score: 0.7, note_type: null },
      ],
      elapsed_ms: 20,
    })
    ...
    await waitFor(() => {
      ...
      const resultThree = screen.getByText('Result Three').closest('[class*="cursor-pointer"]')!
  ...
  it('clears stale results when a follow-up search request fails', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('How to Design AI-first APIs')).not.toBeInTheDocument()
      expect(screen.getByText('No results found')).toBeInTheDocument()
    })
  })
