...
function normalizeHeadingTitle(input: { title: string }): string {
  return input.title.trim().replace(/\s+/g, ' ')
}

function sameHeadingTitle(input: HeadingTitlePair): boolean {
  return normalizeHeadingTitle({ title: input.title }) === normalizeHeadingTitle({ title: input.entryTitle })
}
...
function shouldSkipDuplicateTitleHeading(input: DuplicateHeadingCheck): boolean {
  return input.headingIndex === 0
    && input.level === 1
    && sameHeadingTitle({ entryTitle: input.entryTitle, title: input.title })
}
...
function stripFrontmatter(input: { markdown: string }): string {
...
function stripInlineMarkdown(input: { text: string }): string {
  return input.text
    .replace(/\[([^\]]+)\]\([^)]+\)/g, '$1')
    .replace(/\[\[[^|\]]+\|([^\]]+)\]\]/g, '$1')
    .replace(/\[\[([^\]]+)\]\]/g, '$1')
...
function parseMarkdownHeadings(input: { markdown: string }): MarkdownHeading[] {
  return stripFrontmatter({ markdown: input.markdown })
    .split('\n')
...
function blockIdsForMatchingHeadings(input: HeadingBlockMatch): VisibleHeadings {
  const visibleBlocks = visibleHeadingsForEntry(input.entryTitle, parsedBlockHeadings(input.blocks))
  const visibleMarkdown = visibleHeadingsForEntry(input.entryTitle, input.headings)
