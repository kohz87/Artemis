...
export function VaultMenu({
  ...
  menuPlacement = 'above',
  locale = 'en',
  triggerTestId = 'status-vault-trigger',
}: VaultMenuProps) {
  ...
  return (
    <div ref={menuRef} className="vault-menu" style={{ position: 'relative' }}>
      ...
      {open && (
        <div
          className="vault-menu__content"
          style={{
            position: 'absolute',
            ...(menuPlacement === 'below' ? { top: '100%' } : { bottom: '100%' }),
            left: 0,
            marginTop: menuPlacement === 'below' ? 4 : undefined,
            marginBottom: menuPlacement === 'above' ? 4 : undefined,
