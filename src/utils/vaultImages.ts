import { filePreviewAssetSrc } from './filePreviewAsset'
...
const WEB_ASSET_PATHNAME = '/api/vault/asset'
const WEB_ASSET_BASE_URL = 'http://tolaria.local'
...
function assetUrl(path: AbsolutePath): MarkdownImageUrl {
  return filePreviewAssetSrc(path)
}
...
function webAssetPath(url: MarkdownImageUrl): AbsolutePath | null {
  try {
    const parsed = new URL(url, WEB_ASSET_BASE_URL)
    if (parsed.pathname !== WEB_ASSET_PATHNAME) return null
    const isRelativeAssetUrl = parsed.origin === WEB_ASSET_BASE_URL
    const isCurrentOriginAssetUrl = typeof window !== 'undefined' && parsed.origin === window.location.origin
    if (!isRelativeAssetUrl && !isCurrentOriginAssetUrl) return null
...
function decodeAssetPath(url: MarkdownImageUrl): AbsolutePath {
  const webPath = webAssetPath(url)
...
function isAssetUrl(url: MarkdownImageUrl): boolean {
  return webAssetPath(url) !== null || assetUrlPrefix(url) !== null
}
