...
function webAssetUrl(path: string): string {
...
describe('resolveImageUrls', () => {
  it('converts relative attachment paths to web asset URLs outside desktop', () => {
    ...
    expect(resolveImageUrls(markdown, '/vault')).toBe(
      `![alt](${webAssetUrl('/vault/attachments/file.png')})`,
    )
  })
  ...
  it('rewrites web asset URLs from a different vault', () => {
    ...
    expect(resolveImageUrls(markdown, '/Users/john/Documents/Getting Started')).toBe(
      `![CleanShot](${webAssetUrl('/Users/john/Documents/Getting Started/attachments/CleanShot.png')})`,
    )
  })
})
...
describe('portableImageUrls', () => {
  ...
  it('converts web asset attachment URLs to relative paths', () => {
    const url = webAssetUrl('/vault/attachments/web.png')
    const markdown = `![screenshot](${url})`

    expect(portableImageUrls(markdown, '/vault')).toBe(
      '![screenshot](attachments/web.png)',
    )
  })
