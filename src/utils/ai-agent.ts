 ...
 *
 * App-managed sessions can edit files in the active vault and use Artemis-specific
 * local AI context (search_notes, get_vault_context, get_note, open_note).
...
function permissionModeInstructions(
  ...
  if (mode === 'power_user') {
    if (agent === 'pi') {
      return `Power User mode is selected, but Pi currently uses the same conservative Artemis local AI configuration in both modes. Do not promise shell execution unless the Pi CLI exposes it directly in this run.`
    }
  ...
  return `Vault Safe mode is active. Do not use shell, terminal, Bash, Python/Node script execution, git, or command-line tools. If the user asks whether shell commands are available, say they are not available in Vault Safe. Use file/search/edit tools and Artemis local AI context instead.`
}

const AGENT_SYSTEM_PREAMBLE = `You are working inside Artemis, a personal knowledge management app.
...
Use the provided local AI context for: full-text search (search_notes), vault orientation (get_vault_context), parsed note reading (get_note), and opening notes in the UI (open_note).

When you create or edit a note, call open_note(path) so the user sees it in Artemis.
