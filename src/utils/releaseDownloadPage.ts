...
const PLATFORM_METADATA: Record<StablePlatformKey, { buttonLabel: string; label: string }> = {
  ...
  'linux-x86_64': {
    buttonLabel: 'Download Artemis for Linux',
    label: 'Linux',
  },
  'windows-x86_64': {
...
function buildStableDownloadPageContent(
  ...
  if (Object.keys(downloads).length > 0) {
    return {
      ...
      message: 'Preparing the latest stable Artemis download for your platform.',
      shouldRedirect: true,
      title: 'Artemis Stable Download',
    }
  }

  return {
    helperText: 'Use the button below to check the latest release history.',
    message: 'No stable Artemis downloads are available yet.',
    shouldRedirect: false,
    title: 'Artemis Stable Download Unavailable',
  }
}
      ...
      if (resolvedTarget) {
        ...
        if (message) {
          message.textContent = requiresMacChoice
            ...
            : 'Redirecting to the latest stable Artemis download for ' + resolvedTarget.label + '.';
        }
