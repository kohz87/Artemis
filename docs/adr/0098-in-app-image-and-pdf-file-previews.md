...
- Supported images render with `<img>` and supported PDFs render with the webview/browser PDF object renderer, both using the shared vault asset URL helper.
...
- PDF preview fallback content lives inside the PDF object so unsupported or failed renderers still expose an explicit in-app fallback.
...
- Broken PDFs may rely on the webview/browser renderer's own failure state, but the surrounding Tolaria preview chrome still provides copy-path actions.
