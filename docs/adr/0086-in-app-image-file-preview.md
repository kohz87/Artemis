...
- `FilePreview` renders supported image extensions through the shared vault asset URL helper so the original file remains on disk in both desktop and web mode.
- Broken images and unsupported binary files render an explicit in-app fallback state instead of launching another app automatically.
