...
ADR-0016 established opt-in PostHog analytics and Sentry crash reporting, but new feature telemetry now spans file previews, inline image lightbox opens and All Notes visibility toggles. Instrumenting those flows ad hoc would make it easy to leak vault-specific data such as note paths, filenames, prompt text, or image URLs.
...
- Allowed payloads are constrained to categories and counts such as preview kind, action kind, All Notes visibility category/enabled state.
...
- **Option C**: Avoid new product events entirely — safest from a privacy standpoint, but leaves the team blind to whether preview and list-visibility features are actually being used or failing.
