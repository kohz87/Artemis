...
mod tests {
    ...
    fn run_subprocess_closes_stdin_even_when_parent_stdin_pipe_is_open() {
        ...
        let mut child = Command::new(std::env::current_exe().unwrap())
            ...
            .arg("--ignored")
            .arg("--nocapture")
            .env("ARTEMIS_STDIN_PROBE_CHILD", "1")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
    ...
    fn stdin_probe_parent_child() {
        if std::env::var_os("ARTEMIS_STDIN_PROBE_CHILD").is_none() {
            return;
        }
        ...
        std::env::set_var("ARTEMIS_STDIN_PROBE_MOCK_CLAUDE_CHILD", "1");
        let mut events = vec![];
        let result = run_claude_subprocess(&fake_bin, &args, None, &mut |event| events.push(event));
        std::env::remove_var("ARTEMIS_STDIN_PROBE_MOCK_CLAUDE_CHILD");
    ...
    fn stdin_probe_mock_claude_child() {
        if std::env::var_os("ARTEMIS_STDIN_PROBE_MOCK_CLAUDE_CHILD").is_none() {
            return;
        }
