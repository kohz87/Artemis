...
/// Describes Artemis vault mechanics only; no user-specific structure.
...
# AGENTS.md - Artemis Vault

This is an Artemis vault, a folder of Markdown files with YAML frontmatter forming a personal knowledge graph.
...
- Frontmatter properties that start with `_` are usually Artemis-managed state. Leave them alone unless the user explicitly asks for them to change.
...
Saved views live in `views/*.yml` and are written as YAML. Artemis scans every `.yml` file in `views/`, and the filename is the stable view id, so use kebab-case filenames such as `active-projects.yml`.
...
- Relationship filters can use wikilinks in `value`, for example `"[[artemis]]"`.
...
fn refresh_cloned_vault_config_files(vault_path: &Path) -> Result<(), String> {
    ...
    crate::git::git_commit(
        ...
        "Initialize Artemis config files",
    )?;
    Ok(())
}
...
mod tests {
    ...
    fn init_source_repo(path: &Path, agents_content: Option<&str>) {
        ...
        StdCommand::new("git")
            ...
            .unwrap();
        StdCommand::new("git")
            .args(["config", "user.email", "artemis@app.local"])
            .current_dir(path)
            .output()
            .unwrap();
        StdCommand::new("git")
            .args(["config", "user.name", "Artemis App"])
            .current_dir(path)
            .output()
            .unwrap();
        StdCommand::new("git")
...
"#;
    ...
    #[test]
    fn test_agents_template_matches_current_artemis_vault_conventions() {
        assert!(AGENTS_MD.starts_with("---\ntype: Note\n_organized: true\n---\n"));
        assert!(AGENTS_MD.contains("# AGENTS.md - Artemis Vault"));
