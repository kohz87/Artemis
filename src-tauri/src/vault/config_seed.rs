...
/// Also seeds Artemis-managed root type definitions used by repair/bootstrap flows.
...
mod tests {
    ...
    fn assert_refreshes_outdated_managed_agents(run: VaultOperation) {
        let outdated_agents = AGENTS_MD.replacen(
            ...
            "`type:` is the preferred type field. Artemis still understands legacy aliases such as `Is A`.",
            1,
        );
        ...
        assert!(!content.contains("Artemis still understands legacy aliases such as `Is A`."));
    }
    ...
    fn assert_required_agents_file_seeded(vault: &Path) {
        ...
        assert!(read_root_agents(vault).contains("Artemis Vault"));
    }
    ...
    fn test_migrate_agents_md_idempotent_when_no_legacy() {
        ...
        assert!(root.contains("Artemis Vault"));
        assert_eq!(read_root_claude(&vault), CLAUDE_MD_SHIM);
    }
