...
const MCP_SERVER_NAME: &str = "artemis";
const LEGACY_MCP_SERVER_NAME: &str = "tolaria";
const OLDEST_MCP_SERVER_NAME: &str = "laputa";
...
pub(crate) fn configured_ws_port() -> u16 {
    env_port(
        &[
            "ARTEMIS_MCP_WS_PORT",
            "TOLARIA_MCP_WS_PORT",
...
pub(crate) fn configured_ws_ui_port() -> u16 {
    env_port(
        &[
            "ARTEMIS_MCP_WS_UI_PORT",
            "ARTEMIS_MCP_UI_PORT",
            "TOLARIA_MCP_WS_UI_PORT",
            "MCP_WS_UI_PORT",
            "WS_UI_PORT",
        ],
        9711,
...
pub fn bridge_info() -> McpBridgeInfo {
    let ws_port = configured_ws_port();
    let ui_port = configured_ws_ui_port();
    McpBridgeInfo {
        ws_port,
        ui_port,
        ws_url: format!("ws://localhost:{ws_port}"),
        ui_ws_url: format!("ws://localhost:{ui_port}"),
        available: true,
    }
}
...
fn verify_node_version(node: &Path) -> Result<(), String> {
    ...
    if major < 18 {
        return Err(format!(
            "Node.js 18+ is required for Artemis MCP tools; found {}",
            raw_version.trim()
        ));
    }
...
fn linux_package_mcp_server_dirs(root: &Path) -> Vec<PathBuf> {
    vec![
        root.join("Artemis").join("mcp-server"),
        root.join("Artemis").join("resources").join("mcp-server"),
        root.join("lib").join("artemis").join("mcp-server"),
        root.join("lib")
            .join("artemis")
            .join("resources")
            .join("mcp-server"),
...
pub fn spawn_ws_bridge(vault_path: impl AsRef<Path>) -> Result<Child, String> {
    ...
    let ws_port = configured_ws_port().to_string();
    let ws_ui_port = configured_ws_ui_port().to_string();

    let child = crate::hidden_command(node)
        .arg(&script)
        .env("VAULT_PATH", vault_path)
        .env("WS_PORT", &ws_port)
        .env("WS_UI_PORT", &ws_ui_port)
    ...
    config
        ...
        .and_then(|servers| {
            servers
                ...
                .or_else(|| servers.get(OLDEST_MCP_SERVER_NAME))
        })
        .cloned()
}
...
fn entry_has_ui_port(entry: &serde_json::Value) -> bool {
    let expected = configured_ws_ui_port().to_string();
    entry["env"]["WS_UI_PORT"].as_str() == Some(expected.as_str())
}
...
pub fn mcp_config_snippet(vault_path: &str) -> Result<String, String> {
    let node = find_node().map_err(|e| {
        format!("Node.js 18+ is required on PATH before Artemis can build MCP config: {e}")
    })?;
    let server_dir = mcp_server_dir()?;
...
/// Register Artemis as an MCP server in external AI tool config files.
pub fn register_mcp(vault_path: &str) -> Result<String, String> {
    let node = find_node().map_err(|e| {
        format!("Node.js 18+ is required on PATH before Artemis can register MCP tools: {e}")
    })?;
    let server_dir = mcp_server_dir()?;
    let index_js = server_dir.join("index.js").to_string_lossy().into_owned();
    let node_command = node.to_string_lossy().into_owned();
...
/// Insert or update the Artemis entry in an MCP config file.
fn upsert_mcp_config(config_path: &Path, entry: &serde_json::Value) -> Result<bool, String> {
    ...
    let was_update = servers.get(MCP_SERVER_NAME).is_some()
        || servers.get(LEGACY_MCP_SERVER_NAME).is_some()
        || servers.get(OLDEST_MCP_SERVER_NAME).is_some();
    servers.remove(LEGACY_MCP_SERVER_NAME);
    servers.remove(OLDEST_MCP_SERVER_NAME);
    servers.insert(MCP_SERVER_NAME.to_string(), entry.clone());
...
fn remove_mcp_from_config(config_path: &Path) -> Result<bool, String> {
    ...
    let removed_oldest = servers.remove(OLDEST_MCP_SERVER_NAME).is_some();
    if !removed_primary && !removed_legacy && !removed_oldest {
        return Ok(false);
    }
...
///
/// Returns `Installed` when the Artemis entry exists for the active vault in
...
mod tests {
    ...
    fn assert_registered_artemis_server(
    ...
    #[test]
    fn build_mcp_config_snippet_wraps_artemis_server_entry() {
    ...
    fn mcp_server_dir_candidates_prefer_exe_dir_before_macos_resources() {
        ...
        let exe_path = Path::new("/Users/tester/AppData/Local/Artemis/artemis.exe");
        let candidates = mcp_server_dir_candidates(dev_path, exe_path, None);

        let windows_dir = PathBuf::from("/Users/tester/AppData/Local/Artemis/mcp-server");
    ...
    fn mcp_server_dir_candidates_include_linux_package_resource_roots() {
        let dev_path = Path::new("/repo/mcp-server");
        let exe_path = Path::new("/usr/local/artemis/artemis");
        let candidates = mcp_server_dir_candidates(dev_path, exe_path, None);
        let expected = [
            PathBuf::from("/usr/local/Artemis/mcp-server"),
            PathBuf::from("/usr/local/Artemis/resources/mcp-server"),
            PathBuf::from("/usr/local/lib/artemis/mcp-server"),
            PathBuf::from("/usr/local/lib/artemis/resources/mcp-server"),
            PathBuf::from("/usr/lib/artemis/mcp-server"),
            PathBuf::from("/usr/lib/artemis/resources/mcp-server"),
    ...
    fn mcp_server_dir_candidates_include_linux_appimage_resource_root() {
        let dev_path = Path::new("/repo/mcp-server");
        let exe_path = Path::new("/tmp/.mount_artemis/usr/bin/artemis");
        let appdir = Path::new("/tmp/.mount_artemis");
        let candidates = mcp_server_dir_candidates(dev_path, exe_path, Some(appdir));

        assert!(candidates.contains(&PathBuf::from(
            "/tmp/.mount_artemis/usr/lib/artemis/resources/mcp-server"
        )));
    }
    ...
    fn upsert_creates_new_config() {
        ...
        assert_registered_artemis_server(
            &config,
            ExpectedMcpServer {
    ...
    fn upsert_migrates_legacy_server_name() {
        ...
        let existing = serde_json::json!({
            "mcpServers": {
                    "tolaria": {
                        "command": "node",
                        "args": ["/old-tolaria/index.js"],
                        "env": { "VAULT_PATH": "/old-tolaria" }
                    },
                    "laputa": {
                    "command": "node",
                    "args": ["/old/index.js"],
        ...
        assert!(config["mcpServers"][OLDEST_MCP_SERVER_NAME].is_null());
        assert_eq!(
            config["mcpServers"][MCP_SERVER_NAME]["args"][0],
            "/test/index.js"
        );
    ...
    fn remove_mcp_from_config_removes_primary_and_legacy_entries() {
        ...
        let config = serde_json::json!({
            "mcpServers": {
                "artemis": { "command": "node", "args": ["/index.js"] },
                "tolaria": { "command": "node", "args": ["/legacy-index.js"] },
        ...
        assert!(updated["mcpServers"][OLDEST_MCP_SERVER_NAME].is_null());
