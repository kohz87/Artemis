    ...
    F: FnMut(AiAgentStreamEvent),
{
    let last_message_dir = tempfile::Builder::new()
        .prefix("artemis-codex-last-message-")
        .tempdir()
...
mod tests {
    ...
    fn build_codex_args_can_request_last_message_output_file() {
        if let Ok(args) = build_codex_args(
            ...
            Some(Path::new("/tmp/artemis-codex-last-message.txt")),
        ) {
            assert!(args.windows(2).any(|window| window
                == [
                    "--output-last-message",
                    "/tmp/artemis-codex-last-message.txt",
                ]));
        }
    }
...
"#,
    ...
    fn run_codex_agent_stream_closes_stdin_even_when_parent_stdin_pipe_is_open() {
        ...
        let mut child = std::process::Command::new(std::env::current_exe().unwrap())
            ...
            .arg("--ignored")
            .arg("--nocapture")
            .env("ARTEMIS_CODEX_STDIN_PROBE_PARENT_CHILD", "1")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
    ...
    fn codex_stdin_probe_parent_child() {
        if std::env::var_os("ARTEMIS_CODEX_STDIN_PROBE_PARENT_CHILD").is_none() {
            return;
        }
