...
fn normalize_settings(settings: Settings) -> Settings {
    Settings {
        ...
        word_wrap_enabled: settings.word_wrap_enabled,
    }
}
...
mod tests {
    ...
    fn test_settings_json_roundtrip() {
        let settings = Settings {
            ...
            note_width_overrides: Some(HashMap::from([(
                "Notes/Long.md".to_string(),
                "wide".to_string(),
            )])),
    ...
    fn test_save_and_load_preserves_values() {
        ...
        assert_eq!(
            loaded
                .note_width_overrides
                .as_ref()
                .and_then(|overrides| overrides.get("Notes/Long.md"))
                .map(String::as_str),
            Some("wide")
        );
        ...
        assert_eq!(loaded.word_wrap_enabled, Some(false));
    }
