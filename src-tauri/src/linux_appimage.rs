...
const ARTEMIS_COLRV1_FONTCONFIG_FILE: &str = "artemis-appimage-no-colrv1-emoji.conf";
    ...
    E: FnMut(&str) -> bool,
{
    ...
    if has_non_empty_env(&mut get_var, "LD_PRELOAD")
        || get_var("ARTEMIS_APPIMAGE_WAYLAND_PRELOAD_ATTEMPTED").is_some_and(|value| value == "1")
...
fn apply_colrv1_emoji_font_guard() {
    ...
    let Some(config_path) = colrv1_fontconfig_file_path() else {
        eprintln!("Artemis AppImage COLRv1 font guard skipped: failed to resolve cache directory");
        return;
    };
    ...
    if let Err(error) = std::fs::create_dir_all(parent) {
        eprintln!("Artemis AppImage COLRv1 font guard skipped: failed to prepare cache ({error})");
        return;
    }

    if let Err(error) = std::fs::write(&config_path, colrv1_emoji_fontconfig_contents(&font_path)) {
        eprintln!("Artemis AppImage COLRv1 font guard skipped: failed to write config ({error})");
        return;
    }
...
fn colrv1_fontconfig_file_path() -> Option<std::path::PathBuf> {
    ...
    Some(
        cache_dir
            .join("artemis")
            .join(ARTEMIS_COLRV1_FONTCONFIG_FILE),
    )
}
...
fn apply_wayland_client_preload() {
    ...
    let exe = match std::env::current_exe() {
        ...
        Err(e) => {
            eprintln!(
                "Artemis AppImage Wayland preload skipped: failed to resolve executable ({e})"
            );
            return;
        }
    };
    ...
    let error = std::process::Command::new(exe)
        ...
        .env("ARTEMIS_APPIMAGE_WAYLAND_PRELOAD_ATTEMPTED", "1")
        .exec();
    eprintln!("Artemis AppImage Wayland preload skipped: failed to re-exec ({error})");
}
...
mod tests {
    ...
    fn startup_env_overrides_disable_unstable_webkit_rendering_for_appimages() {
        let overrides = startup_env_overrides_with(|key| match key {
            "APPIMAGE" => Some("/tmp/Artemis.AppImage".to_string()),
            _ => None,
        });
    ...
    fn startup_env_overrides_preserve_explicit_user_setting_per_variable() {
        let overrides = startup_env_overrides_with(|key| match key {
            "APPDIR" => Some("/tmp/.mount_Artemis".to_string()),
