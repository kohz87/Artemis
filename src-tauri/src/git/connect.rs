...
impl RemoteConnection {
    ...
    fn pushed_history_message(&self) -> String {
        format!(
            "Remote connected. Artemis pushed your local commits and is now tracking {}.",
            self.remote_branch
        )
    }
...
pub fn git_add_remote(vault_path: &str, remote_url: &str) -> Result<GitAddRemoteResult, String> {
    ...
    if branch.is_empty() {
        return Ok(connect_result(
            ConnectStatus::Error,
            "Artemis could not determine the current branch for this vault.",
        ));
    }
...
fn finish_remote_connection(vault: &Path, connection: &RemoteConnection) -> GitAddRemoteResult {
    ...
    if behind > 0 {
        return connect_result(
            ConnectStatus::IncompatibleHistory,
            format!(
                "This repository already has commits on '{}' that are not in this vault. Artemis will not connect it automatically.",
                connection.branch
            ),
        );
    }
...
fn push_with_tracking(
    ...
    match run_git(vault, &push_args) {
        ...
        Err(stderr) => {
            ...
            {
                return match run_git(vault, &push_args) {
                    Ok(()) => connect_result(ConnectStatus::Connected, success_message),
                    Err(retry_stderr) => classify_connect_error(&retry_stderr),
                };
            }
            classify_connect_error(&stderr)
        }
    }
}
...
fn classify_connect_error(stderr: &str) -> GitAddRemoteResult {
    ...
    if is_checked_out_branch_push_error(&lower) {
        return connect_result(
            ConnectStatus::Error,
            "That local remote is a checked-out repository. Artemis tried to enable local-folder pushes, but the remote still refused the update. Use a bare repository or set receive.denyCurrentBranch=updateInstead in the remote.",
        );
    }
...
fn local_origin_path(vault: &Path) -> Option<PathBuf> {
    let output = git_command()
        .args(["remote", "get-url", DEFAULT_REMOTE_NAME])
        .current_dir(vault)
        .output()
        .ok()?;
    if !output.status.success() {
