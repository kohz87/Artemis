...
pub fn git_pull(vault_path: &str) -> Result<GitPullResult, String> {
    ...
    if !ensure_pull_upstream(vault) {
        return Ok(GitPullResult {
            status: "up_to_date".to_string(),
            message: "No upstream branch exists yet. Push first to publish this branch, then pull will track it.".to_string(),
            updated_files: vec![],
            conflict_files: vec![],
        });
    }
...
fn ensure_pull_upstream(vault: &Path) -> bool {
    if branch_has_upstream(vault) {
        return true;
    }

    let Ok(branch) = current_branch(vault) else {
        return false;
    };
    if branch.is_empty() {
        return false;
    ...
    let remote_ref_exists = git_command()
...
fn branch_has_upstream(vault: &Path) -> bool {
    git_command()
...
pub fn classify_push_error(stderr: &str) -> GitPushResult {
    ...
    if is_checked_out_branch_push_error(&lower) {
        return push_error(
            "error",
            "Push failed: the local remote is a checked-out repository. Artemis tried to enable local-folder pushes, but the remote still refused the update. Use a bare repository or set receive.denyCurrentBranch=updateInstead in the remote.",
        );
    }
...
fn is_checked_out_branch_push_error(lower: &str) -> bool {
...
pub fn git_push(vault_path: &str) -> Result<GitPushResult, String> {
    let vault = Path::new(vault_path);
    let branch = current_branch(vault).unwrap_or_default();

    let output = run_push_command(vault, &branch)?;
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        if is_checked_out_branch_push_error(&stderr.to_lowercase())
            && configure_local_remote_for_checked_out_push(vault)
        {
            let retry = run_push_command(vault, &branch)?;
            if retry.status.success() {
                return Ok(GitPushResult {
                    status: "ok".to_string(),
                    message: "Pushed to local remote".to_string(),
                });
            }
            let retry_stderr = String::from_utf8_lossy(&retry.stderr);
            return Ok(classify_push_error(&retry_stderr));
        }
...
fn run_push_command(vault: &Path, branch: &str) -> Result<Output, String> {
    let mut command = git_command();
    command.current_dir(vault);
    if branch.is_empty() || branch_has_upstream(vault) {
        command.args(["push"]);
    } else {
        command.args(["push", "--set-upstream", "origin"]).arg(&branch);
    }
...
fn configure_local_remote_for_checked_out_push(vault: &Path) -> bool {
    let Some(remote_path) = local_origin_path(vault) else {
        return false;
    };
...
fn local_origin_path(vault: &Path) -> Option<PathBuf> {
    let output = git_command()
    ...
    local_remote_path(vault, String::from_utf8_lossy(&output.stdout).trim())
}

fn local_remote_path(vault: &Path, remote_url: &str) -> Option<PathBuf> {
