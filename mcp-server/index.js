#!/usr/bin/env node
/**
 * Artemis MCP Server — lightweight vault tools for AI agents.
 *
 * These MCP tools provide Artemis-specific capabilities alongside each
 ...
 *   - open_note: signal Artemis UI to open a note as a tab
...
const WS_UI_PORT = envPort(['WS_UI_PORT', 'ARTEMIS_MCP_WS_UI_PORT', 'ARTEMIS_MCP_UI_PORT', 'MCP_WS_UI_PORT'], 9711)
...
const TOOLS = [
  ...
  {
    name: 'open_note',
    description: 'Open a note in the Artemis UI as a new tab. Use after creating or editing a note so the user can see it.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Visually highlight a UI element in Artemis (editor, tab, properties panel, or note list). The highlight auto-clears after a short delay.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'refresh_vault',
    description: 'Trigger a vault rescan so new or modified files appear immediately in the Artemis note list.',
    inputSchema: {
      type: 'object',
      properties: {
...
function handleOpenNote(args) {
  ...
  return { content: [{ type: 'text', text: `Opening ${args.path} in Artemis` }] }
}
...
const server = new Server(
  { name: 'artemis-mcp-server', version: '0.3.0' },
  { capabilities: { tools: {} } },
)
...
async function main() {
  ...
  console.error(`Artemis MCP server running (vault: ${VAULT_PATH})`)
}
